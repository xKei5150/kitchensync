...
# to Cloud Run on the PROD project and wires planShoppingAllocation to it.
#
...
# Preserve existing prod env values (admin/invite/privacy parameters); only
# replace the planner lines.
ENV_FILE="functions/.env.${PROJECT}"
touch "$ENV_FILE"
grep -v '^PLANNER_URL=\|^PLANNER_AUDIENCE=' "$ENV_FILE" > "$ENV_FILE.tmp" || true
{
  cat "$ENV_FILE.tmp"
  echo "PLANNER_URL=${URL}"
  echo "PLANNER_AUDIENCE=${URL}"
} > "$ENV_FILE"
rm -f "$ENV_FILE.tmp"
echo "   updated PLANNER_* in $ENV_FILE (admin/invite/privacy vars preserved)"
